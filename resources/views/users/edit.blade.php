...
<section class="mb-4">
  <div class="card">
    <div class="card-header py-3">
      <h5 class="mb-0 text-center"><strong>Update Akun</strong></h5>
    </div>
    <div class="card-body">
      <form action="{{ url('akun/' . $users->id) }}" method="post">
        ...
        <select class="form-control" name="role">
          <option {{ old('role', $users->role) == "Admin"? 'selected':''}} value="Admin">Admin</option>
          <option {{ old('role', $users->role) == "Pimpinan"? 'selected':''}} value="Pimpinan">Pimpinan</option>
        </select><br>
