...
<section class="mb-4">
  <div class="card">
    <div class="card-header py-3">
      <h5 class="mb-0 text-center"><strong>Lihat Akun</strong></h5>
    </div>
    <div class="card-body">
      <input type="hidden" name="id" id="id" value="{{$users->id}}" id="id" />
      <label>Nama</label></br>
      <input type="text" name="name" id="name" value="{{$users->name}}" class="form-control" disabled></br>
      <label>Username</label></br>
      <input type="text" name="username" id="username" value="{{$users->username}}" class="form-control" disabled></br>
      <label>Password</label></br>
      <input type="password" name="password" id="password" value="{{$users->password}}" class="form-control" disabled></br>
      <label>Role</label></br>
      <select class="form-control" name="role" disabled>
        <option value="{{$users->role}}" selected>{{$users->role}}</option>
      </select><br>
      <a class="btn btn-success" href="{{ url('/akun') }}" role="button">Kembali</a>
    </div>
    </hr>
  </div>
  </div>
